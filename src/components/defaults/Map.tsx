...
const Map = () => {
  ...
  const { mapData, gameControllerData, mineLocationsData,userData } = useSelector(
  ...
  const[myCurrLocation, setMyCurrLocation] = useState<LatLngLiteral | null>(null)
  ...
  const ZoomBtn = ({zoomIn, zoomOut}: ZoomBtnInterface) => {
    return (
      <div className="zoomBtn">
        <div className="zoomInPh" 
        onClick={()=>zoomIn()}
        ></div>
        <div className="zoomOutPh"
        onClick={()=>zoomOut()}
        ></div>
        <svg
        width="60"
  ...
  const increaseZoom = () => {
    dispatch(updateMapDataAction({zoomLevel: mapData.data.zoomLevel  + 0.8} as MapdataType) as any)
  };
  ...
  const getWarningResponse = (response: boolean) => {
    if (response) {
      ...
      setTimeout(()=>{
        dispatch(updateMapDataAction({mapAnimationOngoing: true} as any) as any)
      }, 2000)
      moveLocation();
    } else {
      setState((prev: any) => {
  ...
  const moveLocation = async() => {
   ...
   const doMoveAnimation = async(from: LatLngLiteral, to: LatLngLiteral, cleanUp?:()=>void) =>{
      ...
      <div id="MapCon">
        ...
        >
         ...
         <Circle 
          ...
          {mineLocationsData?.data?.map?.((i: any, idx: number) => {
            return (
              i.location_type === "base" && i.owner_id === userId  &&
