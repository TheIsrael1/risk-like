        ...
        <div className={`commentsBackdrop ${commentsOpen ? `show` : ``}`}>
            <div className="commentModal">
                <div className="modalTitle">
                    ...
                    <img width={10} src={cancelBtn} alt="cancel" className="cancelBtn" 
                ...
                <div className="commentNav">
                    <div className={`commentNavItem ${state.currNav === "friends" ? `active` : ``}`}
                    onClick={()=>changeNav("friends")}
                    >
                    FRIENDS
                    </div>
                    <div className={`commentNavItem  ${state.currNav === "chats" ? `active` : ``}`}
                ...
                {dummyChatDetails.map((detail: any, idx: number)=>{
                   return detail.onlineStatus === "online" &&
                   <ChatDetailRow 
                ...
                {dummyChatDetails.map((detail: any, idx: number)=>{
                   return detail.onlineStatus === "offline" &&
                   <ChatDetailRow 
                   chatName={detail.chatName}
                   img={detail.img}
                   key={idx}
                ...
                {dummyChatDetails.map((detail: any, idx: number)=>{
                   return detail.datePosted === "today" &&
                   <ChatDetailRow 
                   chatName={detail.chatName}
                   img={detail.img}
                ...
                {dummyChatDetails.map((detail: any, idx: number)=>{
                   return detail.datePosted === "yesterday" &&
                   <ChatDetailRow 
                   chatName={detail.chatName}
                   img={detail.img}
                   chatSummary={detail.chatSummary}
