...
const Radar = () => {
  ...
  const retrieveCoords = useCallback(() => {
    ...
    const cartCord = neigbours?.map((loc: any) =>
      getCartesianCoord(loc.lat, loc.lng)
    );
    const relativeCoord = cartCord?.map((c: any) => {
      return {
        x: c[0] - centerCartCoord[0],
        y: c[1] - centerCartCoord[1],
        z: c[2] - centerCartCoord[2],
      };
    });
    //
    ...
    <div id="Radar">
      <span className="span">
        {gameControllerData?.data?.lat
          ? `Your radar detects ${neigbourCount} ${
              neigbourCount === 1 ? `location` : `locations`
            } close by`
          : `Select a target location`}
      </span>
      <img loading="lazy" className="radar" src={radar} alt="radar" />
    </div>
  );
};
