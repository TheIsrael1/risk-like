...
const MineDetailsDropdown = ({ item, img }: MineDetailsDropdownInterface) => {
  ...
  return (
    <div
      className={`mineCard ${open && `active`}`}
      ref={ref}
      onClick={() => toggle(true)}
    >
      <div className="mineName">{item?.location?.name}</div>
      <div className="rateCon">
        <span className="rateH">Minning Rate:</span>
        ...
        <div className={`detailsDrop ${open && `open`}`}>
          ...
          >
            <div className="detailsTop">
              ...
              <div>
                <span className="rtkValue">{item?.rtkValue ?? "N/A"}</span>
              </div>
            </div>
            <div className="detailsRow">
              <span className="detailTitle">Mine Type:</span>
              <span className="detailValue bold">
                <img loading="lazy" width={40} src={img} alt="mineTpe" />
              </span>
            </div>
            <div className="detailsRow">
              <span className="detailTitle">Mine Level:</span>
              <span className="detailValue bold">
                {item?.mineLevel ?? "N/A"}
              </span>
            </div>
            <div className="detailsRow">
              <span className="detailTitle">Mining Rate:</span>
              <span className="detailValue bold">
                {getValueByKey(item?.location.properties, "production_rate")}{" "}
                {getValueByKey(item?.location.properties, "mine_type")}/hr
              </span>
            ...
            <div className="detailsRow">
              ...
              <span className="detailValue bold">{locationName ?? "N/A"}</span>
            </div>
            <div className="detailsRow">
              <span className="detailTitle">Decomissioned:</span>
              <span className="detailValue bold">
