...
const LocationSelectionModal = React.memo(
  ({ setAttackForce }: LocationSelectionModalInterface) => {
    ...
    const updateCurrSelection = (id: number) => {
      const selection = mineLocationsData.data?.find?.(
        (item) => item.id === id
      );
    ...
    return (
      <div id="LocationSelectionModal">
        <div className="dropBtn" onClick={() => toggle()}>
          <span className="dropBtnText">{currentSelection?.name}</span>
          <img
            loading="lazy"
            width={12}
            alt="dropArrow"
        ...
        <div className="locationModalCon">
          <div
            className={`locationModal ${modalOpen ? `showLocationModal` : ``}`}
          >
            <div className="locationModalTop">
              <span className="locationModalTopText">Your Locations</span>
              <img
                loading="lazy"
                width={14}
                src={cancel}
